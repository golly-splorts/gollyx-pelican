...
if [ -z ${GOLLYX_DRAGON_PELICAN_HOME+x} ]; then
	echo 'You must set the $GOLLYX_DRAGON_PELICAN_HOME environment variable to proceed.'
    echo 'Try sourcing environment.{STAGE}'
	exit 1
else 
	echo "\$GOLLYX_DRAGON_PELICAN_HOME is set to '$GOLLYX_DRAGON_PELICAN_HOME'"
fi
...
case ${GOLLYX_STAGE} in
    ...
    integration)
    DOM="${GOLLYX_BASE_UI}"
    REPO="dragon.golly.life-integration"
    ;;
    prod)
    DOM="${GOLLYX_BASE_UI}"
    REPO="dragon.golly.life"
    ;;
    *)
...
(
cd ${GOLLYX_DRAGON_PELICAN_HOME}/pelican
rm -fr output
